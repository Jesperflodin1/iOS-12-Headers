//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Oct 15 2018 10:31:50).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import "MapsLargerHitTargetButton.h"

#import "UIFocusEnvironment-Protocol.h"
#import "UIGestureRecognizerDelegate-Protocol.h"

@class CALayer, NSArray, NSString, UIBezierPath, UILongPressGestureRecognizer, UITapGestureRecognizer, UIView;
@protocol UIFocusEnvironment, UIFocusItemContainer;

__attribute__((visibility("hidden")))
@interface CarPanButton : MapsLargerHitTargetButton <UIGestureRecognizerDelegate, UIFocusEnvironment>
{
    UILongPressGestureRecognizer *_longPressRecognizer;
    UITapGestureRecognizer *_tapRecognizer;
    UILongPressGestureRecognizer *_physicalLongPressRecognizer;
    UITapGestureRecognizer *_physicalTapRecognizer;
    CALayer *_backgroundLayer;
    UIBezierPath *_touchAreaExtended;
    long long _direction;
}

+ (id)arrowImageForDirection:(long long)arg1;
@property(readonly, nonatomic) long long direction; // @synthesize direction=_direction;
- (void).cxx_destruct;
- (_Bool)gestureRecognizerShouldBegin:(id)arg1;
- (void)layoutSubviews;
- (_Bool)gestureRecognizer:(id)arg1 shouldRecognizeSimultaneouslyWithGestureRecognizer:(id)arg2;
- (_Bool)canBecomeFocused;
- (void)didUpdateFocusInContext:(id)arg1 withAnimationCoordinator:(id)arg2;
@property(readonly, copy) NSString *description;
@property(readonly, nonatomic) UIBezierPath *touchAreaExtended; // @synthesize touchAreaExtended=_touchAreaExtended;
- (void)setCharge:(float)arg1;
- (_Bool)_isChargeEnabled;
- (struct CGSize)sizeThatFits:(struct CGSize)arg1;
- (void)handleTap:(id)arg1;
- (void)handleLongPress:(id)arg1;
- (id)initWithDirection:(long long)arg1;
- (_Bool)pointInside:(struct CGPoint)arg1 withEvent:(id)arg2;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, nonatomic) id <UIFocusItemContainer> focusItemContainer;
@property(readonly) unsigned long long hash;
@property(readonly, nonatomic) __weak id <UIFocusEnvironment> parentFocusEnvironment;
@property(readonly, copy, nonatomic) NSArray *preferredFocusEnvironments;
@property(readonly, nonatomic) __weak UIView *preferredFocusedView;
@property(readonly) Class superclass;

@end

